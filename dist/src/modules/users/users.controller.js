...
let UsersController = class UsersController {
    ...
    async update(id, updateUserDto, file, req) {
        this.checkUserAccess(req.user, id);
        if (file) {
    ...
    async uploadProfileImage(id, req, file) {
        this.checkUserAccess(req.user, id);
        const imageUrl = `${req.protocol}://${req.get('host')}/public/uploads/images/${file.filename}`;
        await this.usersService.update(id, { profileImage: imageUrl });
        return { imageUrl };
    }
    checkUserAccess(user, targetUserId) {
        if (user.id !== targetUserId) {
            throw new common_1.ForbiddenException('You can only modify your own profile');
        }
    }
};
